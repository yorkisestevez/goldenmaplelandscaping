        ...
        <div className="container-custom">
          <Reveal className="text-center max-w-3xl mx-auto mb-32">
            ...
            <h1 className="font-display text-5xl md:text-8xl font-light text-brand-bonewhite leading-[1.05] mb-12">
              ...
              <span className="italic text-brand-gold">in Barrie ON.</span>
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-40">
            {SERVICE_CARDS.map((service, idx) => (
              ...
              >
                <div className="w-16 h-16 bg-brand-nearblack flex items-center justify-center rounded-[2px] text-brand-gold mb-10 group-hover:bg-brand-gold group-hover:text-brand-nearblack transition-colors duration-500 border border-brand-dim/10">
                  <service.icon size={28} strokeWidth={1.5} />
                </div>
                <h2 className="font-display text-3xl font-light text-brand-bonewhite mb-6 group-hover:text-brand-gold transition-colors leading-tight">{service.title}</h2>
                <p className="font-sans text-[15px] text-brand-muted leading-relaxed mb-10 flex-1 font-light">
                  {service.desc}
                </p>
                <Link 
                  to={service.link} 
                  className="flex items-center gap-4 text-brand-gold font-sans text-xs uppercase tracking-[0.2em] group-hover:gap-6 transition-all font-medium"
                >
