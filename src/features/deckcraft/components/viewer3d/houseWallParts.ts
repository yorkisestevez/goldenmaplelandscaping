...
export function houseWallParts(span:number,height:number,openings:HouseOpening[]):Box[]{
 ...
 const xs=[...new Set([-span/2,span/2,...rects.flatMap(r=>[r.left,r.right]).filter(x=>x>-span/2&&x<span/2)])].sort((a,b)=>a-b);
 ...
 for(let yi=0;yi<ys.length-1;yi++)for(let xi=0;xi<xs.length-1;xi++){
  ...
  if(rects.some(r=>x>r.left&&x<r.right&&y>r.bottom&&y<r.top))continue;
