...
export function houseLayout(data:DeckData,width:number){
  ...
  const config=getHouseConfig(data),minX=width/2-config.widthFt*6,maxX=width/2+config.widthFt*6,depth=config.depthFt*12,wallHeight=config.storeys*config.storeyHeightIn,roofRise=config.roofShape==='Flat'?6:Math.min(maxX-minX,config.roofShape==='Hip'?depth:Infinity)*.24;
